...
using namespace std;
	...
	bool Face::onSegment(Point p, Point q, Point r){
	...
	int Face::orientation(Point p, Point q, Point r){
	...
	bool Face::doIntersect(Point p1, Point q1, Point p2, Point q2){
	...
	bool Face::isInside(Point polygon[], int n, Point p){
	...
	void Face::addFirstPoint(Point3 &point){
		// add First Point
		if(m_isComplete){
	...
	bool Face::addPoints(Point3 &point1, Point3 &point2){
		// adds Point
		if(m_isComplete){
			cout<<"\nERROR: Face Already Complete\n";
			return false;
		}
		if(!(lastPoint.isSame(point1))){
			cout<<"\nERROR: Illegal Point Insertion\n";
			return false;
		}
		else{
			if(m_numOfsides == 3){
			...
			if(m_numOfsides > 3){
				double x = (point2.m_coordinates(0));
				double y = (point2.m_coordinates(1));
				double z = (point2.m_coordinates(2));
				...
				if(!((a*x+b*y+c*d) == d)){
					...
					m_allEdges.pb(Edge(point1,point2,m_numOfsides));
					m_numOfsides+=1;
					lastPoint = point2;
					if(point2.isSame(firstPoint)){
						m_isComplete = true;
					}
					else{
						m_allPoints.pb(point2);
					}
					return true;
				}
			}
	...
	bool Face::addEdge(Edge &edge){
		// adds Edge to the Face
		getPlane();
		if(m_isComplete){
			cout<<"\nERROR: Face already complete.";
			return false;
	...
	void Face::getPlane(){
		...
		int p3=2;
		int fl=0;
		while((p1<m_numOfsides) && (p2<m_numOfsides) && (p3<m_numOfsides)){
	...
	Point3 Face::project(Point3 &point,int view){
		// projects a point on the plane acc. to view	
		...
		if(view == 0){
			return Point3(x,y,((d - (a*x) - (b*y))/c),point.m_index);
		}
		else if(view == 1){
			return Point3(((d - (b*y) - (c*z))/a),y,z,point.m_index);
		}
		else{
			return Point3(x,((d - (a*x) - (c*z))/b),z,point.m_index);
		}
	}

   	bool Face::isOnFace(Point3 &point){
	...
	bool Face::isHidden(Point3 &point,int view){
		// checks if the point is hidden
		if(view == 0){// XY
			double x = point.m_coordinates(0);
			double y = point.m_coordinates(1);
			double z = point.m_coordinates(2);
			...
			if(c > 0){
				...
				return false;
					...
					Point3 newPoint = project(point,view);
			...
			cout<<"\nERROR: Illegal passing of view "<<view<<endl;
			return false;
		}
	}

	bool Face::isHiddenEdge(Edge &edge,int view){
		// checks if the edge is hidden
		getPlane();
		for(int i=0;i<m_numOfsides;i++){
	...
	bool Face::isComplete(){
		// checks if the face is complete
		return m_isComplete;
	}
